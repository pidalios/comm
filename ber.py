...
class rxList:
    def __init__(self, BPSKdata, QPSKdata, PSK8data, QAM16data):
        rxList.BPSK = BPSKdata
        rxList.QPSK = QPSKdata
        rxList.PSK8 = PSK8data
        rxList.QAM16 = QAM16data
...
# 8-PSK theoretical value dependency
#def f(x,snr,M):
#    pi = np.pi
...
def plotSER(x, rxList) -> None:
    # Plot SER
    plt.plot(x, rxList.BPSK, marker='o', label='BPSK simulation')
    #plt.plot(x,theoBPSK, marker='o', label='BPSK theoretical')
    plt.plot(x, rxList.QPSK, marker='^', label='QPSK simulation')
    #plt.plot(x,theoQPSK, marker='^', label='QPSK theoretical')
    plt.plot(x, rxList.PSK8, marker='P', label='8PSK simulation')
    #plt.plot(x,theoPSK8, marker='P', label='PSK8 theoretical')
    plt.plot(x, rxList.QAM16, marker='p', label='16QAM simulation')
